...
int fork1(char *name, int (*f)(char *), char *arg, int stacksize, int priority)
{
   ...
   if (Current != NULL)
   {
      if (Current->child_proc_ptr != NULL)
      {
         ...
         {
            pEntry->prev_sibling_ptr = pEntry;
   ...
   if (Current->child_proc_ptr != NULL)
   {
               ...
               Current->child_proc_ptr->next_sibling_ptr != NULL)
      {
         ...
         else if (Current->child_proc_ptr->next_sibling_ptr->status == RECORDED_QUIT)
         {
            ...
            findQuitSib = Current->child_proc_ptr->next_sibling_ptr->next_sibling_ptr;
...
void ListAdd(ProcList *pList, struct proc_struct *pProc)
{
   ...
   else
   {
      if (pList->pHead->parent_proc_ptr != NULL)
      {
         pList->pHead->next_sibling_ptr = pProc;
      }

      else if (pList->pHead->status == ZAPPED)
      {
         pList->pHead->next_zapped_ptr = pProc;
      }
