...
int MboxCreate(int slots, int slot_size)
{
    ...
    // Check if the slot_size is greater that MAX_MESSAGE or if
    // it's less than zero.
    ...
    pMbox->slots = slots;
    pMbox->status = READY;
    pMbox->nextMboxList = slot_list;
    pMbox->recvBlockList = recv_proc_block;
    pMbox->sendBlockList = send_proc_block;
...
int MboxReceive(int mbox_id, void *msg_ptr, int msg_size)
{
    ...
    // Clearing the slot needs work. Causes several tests to fail.
    //memset(&MailSlotTable[pMailbox->mailSlot->mailSlot_id], 0, sizeof(MailSlotTable[pMailbox->mailSlot->mailSlot_id]));
    return retValue;
} /* MboxReceive */
...
int MboxCondSend(int mbox_id, void *msg_ptr, int msg_size)
{
    ...
    {
        if (pMailbox->slots == pMailbox->nextMboxList.count && 
                pMailbox->slots != 0)
        {
            return -2;
        }
