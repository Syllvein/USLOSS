...
/* ------------------------- Prototypes ----------------------------------- */
int check_io();
int MboxCondRecieve(int mbox_id, void *msg_ptr, int msg_size);
...
static void clockHandler2(int dev, void *unit);
static void diskHandler(int dev, void *unit);
static void InitializeHandlers();
static void nullsys(sysargs *args);
static void syscallHandler(int dev, void *unit);
static void termHandler(int dev, void *unit);
...
int MboxCondSend(int mbox_id, void *msg_ptr, int msg_size)
{
    ...
    // Check if we ran out of slots.
    if (pMailbox->slots == slot_list.count)
    {
        return -2;
    }
...
/*static void InitializeHandlers()
{
    int_vec[CLOCK_DEV] = clockHandler2;
    int_vec[DISK_DEV] = diskHandler;
    int_vec[TERM_DEV] = termHandler;
